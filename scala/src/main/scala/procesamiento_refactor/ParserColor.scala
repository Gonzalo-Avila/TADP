...
object ParserColor extends Parser[Nodo] {

  def apply(cadena: String) = {
    val resultado = (((string("color") <> char('[').map { char => char.toString }).map { tupla => Tuple2(List(tupla._1), List(tupla._2)) }) <> ((double <|> integer.map {i => i.toDouble}).sepBy(string(",")).satisfies(lista => lista.size == 3).map { lista => lista.map { numero => numero.toString } } <> char(']').map { char => List(char.toString)})).apply(cadena)
    ...
    resultado.map { r =>
      ...
      Resultado(Colores(parametros(0).toInt, parametros(1).toInt, parametros(2).toInt,contenidoTransformacion.get.getElementoParseado),cerrarParentesis.get.getCadenaRestante)
    }
