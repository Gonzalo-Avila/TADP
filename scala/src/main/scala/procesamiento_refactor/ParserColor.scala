...
object ParserColor extends Parser[Nodo] {
  ...
  def apply(cadena: String) = {
    ...
    resultado.map { r =>
      ...
      Resultado(Colores(parametros(0).toInt, parametros(1).toInt,
        parametros(2).toInt,contenidoTransformacion.get.getElementoParseado),cerrarParentesis.get.getCadenaRestante)
    }
