...
object ParserEscala extends Parser[Nodo] {
  ...
  def apply(cadena: String) = {
    val resultado = (((string("escala") <> char('[')
    ...
    resultado.map { r =>
      ...
      Resultado(Escala(parametros(0), parametros(1),
        contenidoTransformacion.get.getElementoParseado),cerrarParentesis.get.getCadenaRestante)
    }
