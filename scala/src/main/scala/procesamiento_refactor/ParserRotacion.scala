...
object ParserRotacion extends Parser[Nodo] {
  ...
  def apply(cadena: String) = {
    val resultado = (((string("rotacion") <> char('[').map { char => char.toString }).map { tupla => Tuple2(List(tupla._1), List(tupla._2)) }) <> ((double <|> integer.map {i => i.toDouble}).map { elem => List(elem.toString)}  <> char(']').map { char => List(char.toString)})).apply(cadena)
